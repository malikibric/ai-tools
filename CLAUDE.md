...
- **Framework:** Next.js App Router + TypeScript + Tailwind. Design system: cool "editorial paper" palette (cool paper `#eef1f6` bg, ink `#181c27` text, indigo `#4f46e5` accent) defined as CSS vars in `app/globals.css`; Fraunces (serif display) / Inter (body) / IBM Plex Mono (micro-labels) via `next/font` in `app/layout.tsx`.
...
- **Env vars:** `GOOGLE_API_KEY` (required for AI paths), optional `GOOGLE_MODEL` (default `gemini-3.5-flash-lite`). See `.env.example`.
- **Data:** typed in-memory seed stores, one folder per tool under `lib/tools/<tool>/` (`schema.ts` doubles as the Zod schema + TS type; `store.ts` holds seed data and deterministic computations). Each store's seed data must show a range of outcomes (e.g. adoption evidence has one workflow per level: strong/slipping/at-risk/stalled; drift monitor has healthy/at-risk/broken; shadow scanner has 8 responses with ≥1 real risk flag).
